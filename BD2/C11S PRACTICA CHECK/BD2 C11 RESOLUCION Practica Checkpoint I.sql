-- Necesitamos crear una función que dada la cantidad de unidades pedidas y el nombre de un producto, validemos que tengamos la cantidad de unidades en stock necesaria para cubrir el pedido.

DELIMITER $$
CREATE FUNCTION validar_stock (cantidades_pedidas SMALLINT, producto VARCHAR(40))
RETURNS TINYINT DETERMINISTIC
BEGIN
	RETURN (CASE WHEN (SELECT UnidadesStock - cantidades_pedidas FROM productos WHERE ProductoNombre = producto) >= 0 THEN 1 ELSE 0 END);
END $$

-- SELECT *, validar_stock(10, productos.ProductoNombre) FROM productos;

-- Necesitamos crear una tabla de auditoría para la tabla de facturas. Crear algunos triggers para cada vez que insertamos registros en la tabla, para cada vez que modificamos registros y también para cuando eliminamos. La tabla tiene que tener un id propio, el id de la factura, la acción que se realizó,  el responsable de la misma y la fecha y hora. La tabla se genera una sola vez y queda registrada en nuestra base de datos (no hace falta que esté en los triggers).
...
BEGIN
    ...
    DECLARE cur CURSOR FOR SELECT ProductoID, ProductoNombre, PrecioUnitario FROM Productos WHERE FIND_IN_SET(ProductoID, pproductosids);
